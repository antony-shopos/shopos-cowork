...
function Column({ items, reverse = false }: any) {
  return (
    <div className="relative h-[520px] overflow-hidden">
      {/* Top Fade */}
      <div className="absolute top-0 left-0 w-full h-20 z-10 pointer-events-none bg-gradient-to-b from-[#ffffff] to-transparent" />

      {/* Bottom Fade */}
      <div className="absolute bottom-0 left-0 w-full h-20 z-10 pointer-events-none bg-gradient-to-t from-[#ffffff] to-transparent" />

      {/* Ticker */}
      <div
        className={`flex flex-col gap-6 ${reverse ? 'animate-ticker-down' : 'animate-ticker-up'}`}
      >
        {[...items, ...items].map((t: any, i: number) => (
          <div key={i} className="bg-white border border-black/10 rounded-2xl p-6 shadow-sm">
            <p className="text-black/80 mb-4 leading-relaxed">“{t.text}”</p>
            <div className="font-medium text-sm text-black/70">{t.name}</div>
          </div>
        ))}
      </div>
    </div>
    ...
    <section className="relative bg-[#ffffff] py-24 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-medium mb-4">Real users. Real outcomes.</h2>
